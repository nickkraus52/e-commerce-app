function Filters({ category, search, setCategory, setSearch}){
    const filterStyle = "text-[11px] text-[#EEE5E9] text-shadow-sm border-1 border-[#EEE5E9] bg-[#9a9a9a] p-1 mx-1 rounded-md"
    ...
    return (  
        <>
            <div className="flex justify-center mx-3 pt-4 text-shadow-lg">
                ...
                {(search !== "") ? (
                    <span className={filterStyle}>
                        <button className={xStyle} onClick={() => setSearch("")}> X </button> Keyword: {search} 
                    </span>) 
                : null}              
            </div>
        </>
